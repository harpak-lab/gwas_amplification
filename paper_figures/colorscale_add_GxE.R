...
for (perc_amp in seq(0, 1, length.out = 25)) {
  
  print(glue::glue("Working on {perc_amp} amplification."))
  
  for (amp in seq(1, 3, length.out = 25)) {
...
sim_res_df <- readr::read_csv(
  "./colorscale_add_vs_GxE.csv"
)

sim_res_df <- sim_res_df %>%
  mutate(additive_improvement = (additive_corr - GxE_corr) / GxE_corr)

ggplot(data = sim_res_df, aes(x = perc_amp, y = amp, fill = additive_improvement)) +
  geom_tile() +
