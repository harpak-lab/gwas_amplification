...
make_dec_rule_male_df <- function(
  data_dir, trait, n, m, n_samp, pval_thresh = 1, n_ld_samp = Inf, n_repeat_ld_samps = 1
) {
  
  cv_df <- readr::read_csv(
    file = glue::glue("{data_dir}/all_folds_{trait}_ash_full.csv")
  ) %>%
  ...
  dec_rule <- gamp:::get_sigma_ratio_slope(
    median(cv_df$se_m / cv_df$se_f), n = n, m = m
  )
  
  male_cv_df <- cv_df %>%
    dplyr::mutate(fx_diff = abs(ash_fx_diff)) %>%
    dplyr::mutate(expected_se = dec_rule * fx_diff)
  
  if (n_ld_samp == Inf && pval_thresh < 1) {
    
    male_cv_df <- male_cv_df %>%
      dplyr::filter(p_val < pval_thresh)
    
    pct_under_line <- nrow(male_cv_df %>% dplyr::filter(se_m < expected_se)) / nrow(male_cv_df)
    
  } else if (n_ld_samp < Inf && pval_thresh == 1) {
    
    pct_under_line <- 0
    
    for (i in 1:n_repeat_ld_samps) {
      
      snp_df_samp <- sample_snps(male_cv_df, ld_df, samps_per_block = n_ld_samp)
      pct_under_line_iter <- nrow(snp_df_samp %>% dplyr::filter(se_m < expected_se)) / nrow(snp_df_samp)
      pct_under_line <- pct_under_line + (1 / n_repeat_ld_samps) * pct_under_line_iter
    ...
    for (i in 1:n_repeat_ld_samps) {
      ...
      snp_df_samp <- male_cv_df %>% 
        dplyr::filter(p_val < pval_thresh) %>%
        sample_snps(ld_df, samps_per_block = n_ld_samp)
...
g_igf1 <- ggplot(data = igf1_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .025, color = "orange", size = .5) +
  geom_segment(aes(x = 0, y = 0, xend = .02, yend = .02 * igf1_info$slope), linetype = "dashed") +
  ...
  ylim(0, 1) +
  ggtitle("IGF1") +
  annotate("text", x = .01, y = .4, label = "additive", size = 3) +
  annotate("text", x = .01005, y = .33, label = "better", size = 3)
...
g_igf1_top_hits <- ggplot(data = igf1_info_top_hits$cv_df, aes(x = fx_diff, y = se_m)) +
  ...
  xlim(0, .2) +
  ylim(0, (1/3)) +
  annotate("text", x = .1, y = .155, label = "additive", size = 3) +
  annotate("text", x = .10085, y = .1325, label = "better", size = 3) +
  annotate("text", x = .155, y = .08, label = "GxE", size = 3) +
  annotate("text", x = .155, y = .063, label = "better", size = 3)
...
g_albumin <- ggplot(data = albumin_info$cv_df, aes(x = fx_diff, y = se_m)) +
...
g_fvc <- ggplot(data = fvc_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_hba1c <- ggplot(data = hba1c_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_rbc <- ggplot(data = rbc_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_shbg <- ggplot(data = shbg_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_arm_L <- ggplot(data = arm_fatfree_mass_L_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_arm_R <- ggplot(data = arm_fatfree_mass_R_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_bmi <- ggplot(data = bmi_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_calcium <- ggplot(data = calcium_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_creatinine <- ggplot(data = creatinine_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_diastolicBP <- ggplot(data = diastolicBP_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_eosinophil <- ggplot(data = eosinophil_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_height <- ggplot(data = height_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_hip_circ <- ggplot(data = hip_circ_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_lymphocyte_perc <- ggplot(data = lymphocyte_perc_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_protein_total <- ggplot(data = protein_total_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_pulse_rate <- ggplot(data = pulse_rate_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_systolicBP <- ggplot(data = systolicBP_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_wth <- ggplot(data = wth_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_urate <- ggplot(data = urate_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_urea <- ggplot(data = urea_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_waist_circ <- ggplot(data = waist_circ_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_weight <- ggplot(data = weight_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_whole_body_fat_mass <- ggplot(data = whole_body_fat_mass_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
g_wth_bmi_adj <- ggplot(data = wth_bmi_adj_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
perc_better_vec_top_hits <- perc_better_vec_top_hits[names(perc_better_vec)]
...
perc_better_df <- perc_better_df %>%
  dplyr::mutate(
    group = dplyr::if_else(y %% 2 == 0, "even", "odd")
  )

library(ggbreak)

all_df <- perc_better_df %>% dplyr::filter(criterion == "all")

alpha_vals <- dplyr::if_else(all_df$group == "even", .5, 0)
text_colors <- dplyr::if_else(names(perc_better_vec) == "IGF1", "orange", "black")

g_all_trait_dist <- ggplot(data = all_df) +
  scale_y_continuous(
  ...
  theme(
    ...
    axis.text.y = element_text(color = text_colors),
    axis.title = element_text(size = 11)
  ) +
  ggtitle("All Variants") +
  geom_rect(aes(xmin = -.0275, xmax = Inf, ymin = y - .5, ymax = dplyr::lead(y) - .5, fill = group, alpha = alpha_vals)) +
  scale_x_continuous(limits = c(-.0275,.525), expand = c(0, 0)) +
  geom_point(aes(x = perc_better, y = y)) +
  scale_fill_manual(values = c("grey", "white"))

top_hits_df <- perc_better_df %>% dplyr::filter(criterion == "p < 5e-8")

alpha_vals <- dplyr::if_else(top_hits_df$group == "even", .5, 0)
text_colors <- dplyr::if_else(names(perc_better_vec_top_hits) == "IGF1", "orange", "black")

g_p_val_trait_dist <- ggplot(data = top_hits_df) +
  scale_y_continuous(
  ...
  ggtitle("Top Hits") +
  geom_rect(aes(xmin = -4, xmax = Inf, ymin = y - .5, ymax = dplyr::lead(y) - .5, fill = group, alpha = alpha_vals)) +
  scale_x_continuous(limits = c(-4,105), expand = c(0, 0)) +
  geom_point(aes(x = perc_better, y = y)) +
  scale_fill_manual(values = c("grey", "white"))
...
igf1_fig <- annotate_figure(
 igf1_fig,
 left = textGrob("Standard Error (nmol/L)", rot = 90, vjust = 1, gp = gpar(cex = 1, fontsize = 11)),
 bottom = textGrob("Difference in Context Specific Effects (nmol/L)", gp = gpar(cex = 1, fontsize = 11), hjust = .5))
...
#   res = 300
# )
g1 <- ggplot(data = test_info$cv_df, aes(x = fx_diff, y = se_m)) +
  geom_point(alpha = .15, color = "orange", size = 1) +
...
#   res = 300
# )
g2 <- ggplot(data = test_info$cv_df, aes(x = fx_diff, y = se_m)) +
